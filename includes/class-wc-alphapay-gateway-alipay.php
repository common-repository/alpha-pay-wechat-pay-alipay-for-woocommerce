...
class WC_AlphaPay_Alipay extends WC_Payment_Gateway{


	public function __construct(){
		// The global ID for this Payment method
		$this->id = "alphapay_alipay";

		// The Title shown on the top of the Payment Gateways Page next to all the other Payment Gateways
		$this->method_title='AlphaPay - Alipay';
		...
		// $this->icon = ALPHAPAY_URL. '/assets/images/alipay-logo.png';
	...
	public function get_icon() {
		...
		$icons_str = '<img src="' . ALPHAPAY_URL . '/assets/images/alipay-logo.png" class="right-float" alt="Alipay" />';
	...
	function init_form_fields() {
		$this->form_fields = array (
				...
				'description' => array (
						'title' => 'Description',
						'type' => 'textarea',
						'default' => 'Use Alipay App to scan QR Code to complete payment',
						'desc_tip' => true,
						'css' => 'width:400px',
						'section'=>'default'
				),
	...
	public function process_payment($order_id){
		...
		try {
			if($this->is_app_client()){
				$result = AlphaPay_API::generate_alphapay_order($order,'Alipay',"/api/v1.0/h5_payment/partners/%s/orders/%s");
			}else{
				if('1'==$this->get_option('qrcode_redirect')){
					$result = AlphaPay_API::generate_alphapay_order($order,'Alipay',"/api/v1.0/web_gateway/partners/%s/orders/%s");

				}else{
					$result = AlphaPay_API::generate_alphapay_order($order,'Alipay',"/api/v1.0/gateway/partners/%s/orders/%s");
				}
			}
